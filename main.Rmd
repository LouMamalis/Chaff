...
We measured the mass of `r chaff_summary$n[1]` individuals in total, equal numbers of both males (can i paste in something here to link it automatically?) and females (can i paste in something here to link it automatically?).

We used R [@R-core] with tidyverse packages [@R-tidyverse] for all analyses and the rmarkdown [@markdown1] and bookdown [@bookdown] packages for manuscript preparation.
...
There was a significant difference between the mean masses of the female and male chaffinches. This is demonstrated by the results of the t test where the p value was ... and the difference is also displayed in Figure \@ref(fig:mass-fig).
